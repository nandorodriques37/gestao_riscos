...
- **Portfólio (objetivo → iniciativa → marco):** cinco entidades (`pessoas`, `objetivos`, `iniciativas`, `marcos`, `acoes_risco`) sobre a fábrica genérica `api/_table.ts`, atendidas por **uma** rota catch-all `api/portfolio/[[...path]].ts` (o plano Hobby limita a 12 funções). O front carrega as cinco de uma vez (`GET /api/portfolio`) pelo hook `src/hooks/usePortfolio.ts`; as métricas são funções puras em `src/lib/portfolioMetrics.ts`.
- **Plano de ação:** vive **só** em `acoes_risco` — uma linha por mitigação, editável pelo `AcoesEditor` dentro do `EditModal`. `risk_records.acoes` continua sendo gravado, mas como **resumo derivado** (é o que tabela, busca, Gráficos, CSV e KPI de completude leem); quem deriva é `src/lib/planoDeAcao.ts`, sempre a partir do que persistiu, nunca do que a tela pretendia salvar. `acoes_itens` está **congelado**: ninguém escreve mais nele, e `src/lib/acoes.ts` só o lê para converter o legado. Não apagar a coluna.
